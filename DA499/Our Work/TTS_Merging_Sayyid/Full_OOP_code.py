...
class GenerationAudio:
    ...
    def Extract_text_and_time(self, file_path, output_csv):       
        ...
        for index, line in enumerate(lines):                                                   #لووب على كل سطر 
            ...
            try:
                ...
                if "-" in Time:                                                                 #بنفحص اذا في علامة - بالوقت    
                    ...
                    print(f"ℹ تم إنشاء نهاية مؤقتة للسطر: {line}")                            # هون بنطبع انه عملنا نهاية مؤقتة للسطر
    ...
    def Generate_audio(self, csv_path, out_put_path, voice_file, language="ar"):                     
        ...
        with open(csv_path, "r", encoding="utf-8") as f:                                           # هون بنفتح ملف csv
            ...
            for row in reader:
                ...
                print(f"🎤 توليد الصوت للجملة: {text}")                                          #بنطبع شو الجملة يلي رح نولدها صوتيا  
    ...
    def Delete_silence_from_voices(self, csv_path, input_folder, output_folder):                       
        ...
        for index, row in df.iterrows():                                                       # هون بنعمل لووب على كل سطر بالملف
            ...
            input_path = os.path.join(input_folder, filename)                                  # هون بنعمل مسار للملف الصوتي حسب اسم الملف

            if not os.path.exists(input_path):                                                 # هون بنشيك اذا الملف موجود ولا لا
                print(f" الملف غير موجود: {filename}")
                failed_files.append((filename, "ملف غير موجود"))                              ##  اذا مو موجود بنضيفه لقائمة الفشل
                continue
            ...
            try:
                ...
                correct_voices_after_deleted = AudioSegment(audio.tobytes(), frame_rate=rate,     # هون بنحول الملف الصوتي لبيانات صوتية باستخدام pydub
                                    sample_width=audio.dtype.itemsize, channels=1)   
                audio_chunks = split_on_silence(                                                   # هون بنقسم الصوت حسب السايلنس باستخدام pydub
                    correct_voices_after_deleted, min_silence_len=1300, silence_thresh=-45, keep_silence=500
                )
                audio_processed = sum(audio_chunks) if audio_chunks else correct_voices_after_deleted         # هون بنجمع كل المقاطع الصوتية بعد تقسيمها
                audio_np = np.array(audio_processed.get_array_of_samples())                                   # هون بنحول الصوت لمصفوفة  numpy
                output_path = os.path.join(output_folder, filename)                                           # هون بنعمل مسار للملف الصوتي بعد الحذف
        ...
        if failed_files:
            ...
            for fname, reason in failed_files:
                print(f" - {fname} => {reason}")                                                              #  هون بنطبع الملفات يلي فشلت بعملية الحذف وشو سبب الفشل 
...
class MergingAudio:
    ...
    def Merge_voices_in_one_voice(self, input_folder, output_path):                                                
        ...
        files = [f for f in os.listdir(input_folder) if f.endswith(".wav")]                               # هون بنجيب كل الملفات الصوتية من المجلد
        files = sorted(files, key=extract_number)                                                         # هون بنرتب الملفات حسب الرقم يلي طلعناه من الفنكشن يلي فوق

        merged = AudioSegment.silent(duration=0)                                                          #  هون بنعمل ملف صوتي فاضي
        for filename in files:                                                                            ##  هون بنعمل لووب على كل الملفات الصوتية
            path = os.path.join(input_folder, filename)                                                   ##  هون بنعمل مسار للملف الصوتي حسب اسمه
        ...
        merged.export(output_path, format="wav")                                                         ##  هون بنخزن الملف الصوتي الناتج باستخدام pydub
        print(f"\n تم دمج {len(files)} ملف في: {output_path}")                                          #  ##  هون بنطبع عدد الملفات يلي تم دمجها والمكان يلي تم تخزينها فيه

    ##  فنكشن لحساب مدة الصوت أو الفيديو
    def Duration(self, path):    
        result = subprocess.run(                                                        ##  هون بنستخدم subprocess لتشغيل ffprobe وهي اداة مع ffmpeg 
            ["ffprobe", "-v", "error", "-show_entries", "format=duration",              #  ##  هون بنستخدم ffprobe لحساب المدة
            "-of", "default=noprint_wrappers=1:nokey=1", path],                        ##  هون بنحدد الخيارات  ffprobe
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT                            ##  هون بنحدد مكان تخزين الناتج
        )
        return float(result.stdout.decode().strip())                                    ##  هون بنرجع المدة كعدد عشري وبنسوي ازالة للفراغات بالملف 

    ##  فنكشن  لتعديل سرعة الصوت ليتناسب مع الفيديو
    def Optimize_speed_to_match_video(self, video_path, audio_path, speeded_audio_path):                                 
        video_duration = self.Duration(video_path)                                                                      ##  هون بنحسب مدة الفيديو باستخدامالفنكشن  يلي فوق
        audio_duration = self.Duration(audio_path)                                                                      ##  هون بنحسب مدة الصوت باستخدام الفنكشن  يلي فوق
        print(f" مدة الفيديو: {round(video_duration, 2)} ثانية")                                                  #  هون بنطبع مدة الفيديو
        print(f" مدة الصوت : {round(audio_duration, 2)} ثانية")                                                   #        #  هون بنطبع مدة الصوت
        if audio_duration >= video_duration:                                                                       #  هون بنشيك اذا مدة الصوت اكبر من مدة الفيديو
            speed = round(audio_duration / video_duration, 3)                                                         #ياسيدي هاي عشان نحاول نعرف شو افضل سرعة ممكنة نسرع فيه الصوت قسمنا مدة الصوت على مدة الفيديو 
        ...
        cmd_speed = fr'''ffmpeg -i "{audio_path}" -filter:a "{atempo_filter}" -vn "{speeded_audio_path}" -y'''     ##  هون بنعمل امر ffmpeg لتعديل سرعة الصوت
        os.system(cmd_speed)                                                                                       ##  هون بنشغل الامر باستخدام os.system
        print(" تم تسريع الصوت.")

    ##  فنكشن  لدمج الصوت مع الفيديو
    def Merge_audio_with_video(self, video_path, audio_path, final_output_path):                                                                                   
