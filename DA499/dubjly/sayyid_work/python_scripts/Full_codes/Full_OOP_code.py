...
class GenerationAudio:
    ...
    def Extract_text_and_time(self, file_path, output_csv):
        ...
        with open(
            ...
            writer.writerow(
                ["voices", "start", "end", "text"]
            )  # هون بنكتب اسماء الاعمدة
    ...
    def Delete_silence_from_voices(self, csv_path, input_folder, output_folder):
        ...
        print(
            f" عدد الملفات في CSV: {len(df)}"
        )  ##  طباعة عدد الملفات يلي موجودة بالملف
...
class MergingAudio:
    ...
    def Merge_voices_in_one_voice(self, input_folder, output_path):
        ...
        for filename in files:  ##  هون بنعمل لووب على كل الملفات الصوتية
            ...
            audio = AudioSegment.from_wav(
                path
            )  ##  هون بنقرا الملف الصوتي باستخدام pydub
    ...
    def Optimize_speed_to_match_video(self, video_path, audio_path, speeded_audio_path):
        ...
        print(
            f" مدة الفيديو: {round(video_duration, 2)} ثانية"
        )  #  هون بنطبع مدة الفيديو
        print(
            f" مدة الصوت : {round(audio_duration, 2)} ثانية"
        )  #        #  هون بنطبع مدة الصوت
        if (
            audio_duration >= video_duration
        ):  #  هون بنشيك اذا مدة الصوت اكبر من مدة الفيديو
            speed = round(
        ...
        if speed <= 2.0:  #  هون بنشيك اذا السرعة اقل من 2.0
            ...
            atempo_filter = ",".join(
                steps
            )  #  هون بنجمع الخطوات مع بعض باستخدام الفاصلة
...
class Start_the_TTS_process:
    def __init__(self, base_dir):
        ...
        self.TEXT_INPUT = os.path.join(
            base_dir, "sayyid-work", "test-text", "نص-التجربة.txt"
        )
        self.CSV_PATH = os.path.join(
            base_dir, "sayyid-work", "video_and_csv", "timing_sentences_.csv"
        )
        self.VOICE_SAMPLE = os.path.join(
            base_dir, "sayyid-work", "input-test-voice", "غرباء.wav"
        )
        self.ORIGINAL_AUDIO_FOLDER = os.path.join(
            base_dir, "sayyid-work", "output-test-voice", "original-video-voice"
        )
        self.CLEANED_AUDIO_FOLDER = os.path.join(
            base_dir, "sayyid-work", "output-test-voice", "removed-voices"
        )
        self.MERGED_AUDIO = os.path.join(
            base_dir, "sayyid-work", "output-test-voice", "merged_output.wav"
        )
        self.ADJUSTED_AUDIO = os.path.join(
            base_dir, "sayyid-work", "output-test-voice", "merged_output_adjusted.wav"
        )
        self.VIDEO_INPUT = os.path.join(
            base_dir, "sayyid-work", "video_and_csv", "silent_video.mp4"
        )
    ...
    def run(self):
        ...
        self.generator.Generate_audio(
            self.CSV_PATH, self.ORIGINAL_AUDIO_FOLDER, self.VOICE_SAMPLE
        )

        # 3️⃣ حذف السكوت
        self.generator.Delete_silence_from_voices(
            self.CSV_PATH, self.ORIGINAL_AUDIO_FOLDER, self.CLEANED_AUDIO_FOLDER
        )

        # 4️⃣ دمج المقاطع الصوتية
        self.merger.Merge_voices_in_one_voice(
            self.CLEANED_AUDIO_FOLDER, self.MERGED_AUDIO
        )

        # 5️⃣ تعديل السرعة
        self.merger.Optimize_speed_to_match_video(
            self.VIDEO_INPUT, self.MERGED_AUDIO, self.ADJUSTED_AUDIO
        )

        # 6️⃣ دمج الصوت المعدل مع الفيديو
        self.merger.Merge_audio_with_video(
            self.VIDEO_INPUT, self.ADJUSTED_AUDIO, self.FINAL_VIDEO
        )
...
if __name__ == "__main__":
    base_path = os.path.join(
        "C:\\Users\\sauui\\XTTS-project", "Graduation-Project", "DA499", "dubjly"
    )
